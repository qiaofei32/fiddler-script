...
class Handlers
{
    ...
    */
    ...
    public static RulesOption("Debug JS")
    BindPref("fiddlerscript.rules.DebugJs")
    var m_DebugJs: boolean = false;

    public static RulesOption("Craw Wechat")
    BindPref("fiddlerscript.rules.CrawWechat")
    var m_CrawWechat: boolean = false;
	...
	public static RulesOption("Hide 502s")
	BindPref("fiddlerscript.rules.Hide502s")
	var m_Hide502s: boolean = false;
...
*/
    ...
    static function OnBeforeResponse(oSession: Session) {
		...
		var is_js = false;
        if (oSession.url.EndsWith(".js")|| oSession.oResponse.headers.ExistsAndContains("Content-Type", "application/x-javascript") || oSession.oResponse.headers.ExistsAndContains("Content-Type", "application/javascript")  || oSession.oResponse.headers.ExistsAndContains("Content-Type", "text/javascript"))  {
			is_js = true;
        }
		...
		if (m_Hide502s && oSession.responseCode == 502) {
			oSession["ui-hide"] = "true";
		}
		...
		// 开启JS调试
		if(is_debug_js && m_DebugJs){
			var path = oSession.host + oSession.PathAndQuery;
			path = path.Split('?')[0];
			path = path
				.Replace("/", "-")
				.Replace("?", ",")
			...
			var file_path = "C:\\Temp\\FiddlerJsDebug\\" + path;
			if(File.Exists(file_path)){
				oSession.LoadResponseFromFile(file_path);
			} else {
				//Decoding HTTP response in case it's gzip
				oSession.utilDecodeResponse();
				oSession.SaveResponseBody(file_path);
			}	
		}

		// 开启微信公众号抓取
        var is_craw_wechat = false;
        if (oSession.url.Contains("mp.weixin.qq.com/mp/profile_ext?action=home"))  {
			is_craw_wechat = true;
        }
		if(is_craw_wechat && m_CrawWechat){
			var file_path = "D:\\wechat\\home.txt";
            oSession.utilDecodeResponse();
            oSession.SaveRequest(file_path, true);
		}
